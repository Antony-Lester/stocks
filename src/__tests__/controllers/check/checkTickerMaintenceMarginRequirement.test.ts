...
import checkTickerMaintenceMarginRequirement from '../../../controllers/check/checkTickerMaintenceMarginRequirement';

describe('check Ticker Maintence margin requirement', () => {
  test('returns Maintence margin requirement string if valid', async () => {
    ...
    try {
      ...
      await checkTickerMaintenceMarginRequirement('CAJPY').then(data => {
        expect(data).toBe(1);
      });
  ...
  test('returns undefined if not valid', async () => {
    ...
    try {
      ...
      await createTickersTable();
      await populateTickersTable(tickersData);
      await checkTickerMaintenceMarginRequirement('TEST').then(data => {
